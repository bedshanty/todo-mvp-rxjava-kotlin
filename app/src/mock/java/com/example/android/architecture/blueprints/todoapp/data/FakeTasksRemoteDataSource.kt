...
class FakeTasksRemoteDataSource private constructor() : TasksDataSource {
    ...
    override fun getTasks(): Flowable<List<Task>> =
            Flowable.fromIterable(TASKS_SERVICE_DATA.values).toList().toFlowable()

    override fun getTask(taskId: String): Flowable<Optional<Task?>> {
        val task: Task? = TASKS_SERVICE_DATA[taskId]
    ...
    override fun completeTask(taskId: String) {
        TASKS_SERVICE_DATA.get(taskId).let {
            Task(it!!.title, it.description, it.id, true).let {
                TASKS_SERVICE_DATA.put(taskId, it)
            }
        }
    }
    ...
    override fun activateTask(taskId: String) {
        TASKS_SERVICE_DATA.get(taskId).let {
            Task(it!!.title, it.description, it.id).let {
                TASKS_SERVICE_DATA[taskId] = it
            }
        }
    }
